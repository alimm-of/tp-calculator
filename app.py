...
ВЕРСИЯ = "v14"   # номер релиза — виден на странице
...
def index():
    ...
    if request.method == "POST" and db_ok:
        ...
        текст_имени = (f.get("вид_товара_имя") or "").strip()
        # Защита от рассинхрона кода и имени (частый источник ошибок:
        # текст сменили, а скрытый код остался от прошлого товара).
        # Если введён текст — код ВСЕГДА определяем по имени, игнорируя присланный код.
        if текст_имени:
            тл = текст_имени.lower()
            найден = ""
            for v in ВИДЫ_ТОВАРА:
                if v["имя"].lower() == тл or (v.get("имя_рус") or "").lower() == тл:
                    найден = v["код"]
                    break
            if найден:
                код_товара = найден          # имя однозначно → берём его код
            # если по имени не нашли — оставляем присланный код как есть
