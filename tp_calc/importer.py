...
TABLES = {
    ...
    "ЦеныНаГруз": ("ceny_na_gruz",
        ["склад","вес_от","вес_до","цена","экспресс_цена","по_весу",
         "цена_авиа","авиа_по_объему","цена_бренд","цена_z","цена_бренд_авиа",
         "срок_действия","период"]),
    ...
    "Справочник_тп_Склады": ("sklady", ["ид","код","наименование","город_отправки","страна","только_свои_цены"]),
}
...
# Поля-идентификаторы, где пробел внутри числа = разделитель тысяч (надо убрать)
CODE_COLS = {"вид_товара","группа","тип_упаковки","склад","клиент","вид_тарифа","город_отправки",
             "вид_наценки","магазин","ид","код"}
...
def _norm(col, val):
    ...
    if isinstance(val, str):
        # Убираем неразрывные пробелы и разделители тысяч из кодов/идентификаторов
        val = val.replace("\xa0", "").replace("\u202f", "").strip()
        # Кодовые поля вида "1 688" → "1688" (пробел внутри числа = разделитель тысяч)
        if col in CODE_COLS:
            import re as _re
            if _re.fullmatch(r"[\d\s]+", val):
                val = val.replace(" ", "")
    return val
