...
class PriceRepository:
    ...
    def подобрать_базовую_цену(self, inp, средний_вес: float) -> Optional[PriceRow]:
        ...
        # ЦеныНаГруз — FALLBACK: применяется ТОЛЬКО если цены по виду товара нет
        # (в рабочем движке: «Если нет цены по виду товара» → запрос к ЦеныНаГруз).
        # Приоритеты 6/7, чтобы товарные (4/5) всегда были главнее.
        if not cands:
            cands += self._q_na_gruz(inp, средний_вес, d, приоритет=6)
    ...
    def _q_na_gruz(self, inp, средний_вес, d, приоритет):
        ...
        Ищем строки для конкретного склада И общие (склад = NULL). Конкретный склад
        приоритетнее — ему даём меньший приоритет-номер."""
        try:
            out = []
            # 1) по конкретному складу (приоритетнее)
            rows = self.conn.execute("""
                SELECT * FROM ceny_na_gruz
                WHERE склад = ?
                  AND ? BETWEEN вес_от AND вес_до
                  AND (срок_действия IS NULL OR срок_действия = '' OR ? <= срок_действия)
            """, (inp.склад, средний_вес, d)).fetchall()
            out += [_row_to_price(r, "ЦеныНаГруз (склад)", приоритет) for r in rows]
            # 2) общие (склад не указан) — чуть ниже приоритетом
            rows0 = self.conn.execute("""
                SELECT * FROM ceny_na_gruz
                WHERE склад IS NULL
                  AND ? BETWEEN вес_от AND вес_до
                  AND (срок_действия IS NULL OR срок_действия = '' OR ? <= срок_действия)
            """, (средний_вес, d)).fetchall()
            out += [_row_to_price(r, "ЦеныНаГруз (общая)", приоритет + 0.1) for r in rows0]
            return out
        except Exception:
            return []
